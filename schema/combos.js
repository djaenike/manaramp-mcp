 ...
 * schema/combos.js
 *
 * Commander Spellbook combos as their own collection -- not folded into cards.js's `cards`. A
 ...
 * index.js yet.
 *
 * Keyed by Spellbook's own combo id. `pieces` is oracle_id, taken directly from Spellbook's own
 * `card.oracleId` -- verified live (2026-09-13) that their API already returns a real Scryfall
 * oracle_id per piece, no name matching needed (unlike commander_synergies.js's average_decklist).
 *
 ...
 * is bookkeeping only.
 *
 * Mirrored at manaramp/src/lib/server/schema/combos.ts -- keep both in sync.
 */
...
const COMBOS = {
  ...
  properties: {
    ...
    pieces: { bsonType: "array", items: { bsonType: "string" }, description: "oracle_id per entry in pieces_names, same order. The field schema/indexes.js's multikey index targets." },
