...
const validateAndSubmitTool: ToolDefinition<typeof inputSchema> = {
  ...
  description:
    "Check a decklist -- consistency (card count, singleton, color identity, Commander legality), " +
    "mana curve, curve-out probability, price, and real per-card facts (game_changers_found/" +
    "mass_land_denial_found/extra_turns_found/combos_found/tutors_found/land_ramp_found/" +
    "extra_land_drops_found/token_generators_found/counterspells_found/recursion_found/" +
    "card_draw_found/removal_found) -- and, with submit:true, persist it. See format_guidelines for " +
    "target composition/bracket-rule context to assemble AGAINST before calling this; this tool only " +
    "ever reports facts about a decklist you already have. Every *_found list is a raw fact, not a " +
    "verdict -- judge the actual Commander Bracket (1-5) yourself from those facts plus " +
    "format_guidelines' real criteria, and pass it as bracket_estimate. combos_found's total_cmc/" +
    "unresolved_piece_names: when unresolved_piece_names is non-empty, total_cmc is a floor (some " +
    "pieces aren't in manaramp's card database yet). Editing an EXISTING deck? Call read_deck first " +
    "for its current decklist_text, iterate here with submit:false, then submit:true once with the " +
    "same deck_id.",
  inputSchema,
  handler: async (
    { decklist_text, submit, deck_id, deck_name, deck_type, bracket_estimate, bracket_level_requested, is_public, wincon_summary, general_strategy },
    ctx
  ) => {
    ...
    if (submit) {
      ...
      const deckFields = {
        ...
        bracket: bracket_estimate
          ? { estimate: bracket_estimate, combos_found: facts.combos_found.map((c) => ({ pieces: c.pieces, speed: c.speed })) }
          : null,
