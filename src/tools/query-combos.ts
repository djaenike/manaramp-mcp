...
const queryCombosTool: ToolDefinition<typeof inputSchema> = {
  ...
  description:
    ...
    "the full optimize_deck analysis. Not gated on cards being in manaramp's own card database -- " +
  ...
  handler: async ({ card_names, limit }, ctx) => {
    ...
    return { content: [{ type: "text" as const, text: JSON.stringify({ combos }, null, 2) }] };
  },
};
