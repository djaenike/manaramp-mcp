/**
 * sub-tools/delivery/create_playtest_room.ts
 *
...
async function createPlaytestRoom({ deck_name, commanderNames, deckEntries, decklist_text, wincon_summary, general_strategy, consistency, bracket, price }: CreatePlaytestRoomArgs) {
  ...
  const lobbyData = (await lobbyRes.json()) as any;
  ...
  const humanSeat = liveState.seats?.find((s: any) => s.controller === "human");
  if (!humanSeat) {
