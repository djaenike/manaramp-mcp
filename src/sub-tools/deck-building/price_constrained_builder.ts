/**
 * sub-tools/deck-building/price_constrained_builder.ts
...
interface PriceConstrainedBuildResult {
  commander: string;
  price_limit_usd: number | null;
  nonland_cards_selected: number;
  total_spent_usd: number;
  budget_remaining_usd: number | null;
  note: string;
  deck: Array<{ name: string; category?: string; synergy_pct: number; price_usd: number }>;
}

async function buildDeckByPrice(commander_name: string, price_limit_usd?: number, min_synergy_pct?: number): Promise<PriceConstrainedBuildResult> {
  ...
  const edhrecData = (await edhrecRes.json()) as any;
  ...
  const candidates = new Map<string, { name: string; synergy: number; category: string }>();
  for (const section of cardlists) {
    for (const card of section.cardviews ?? []) {
      ...
      const existing = candidates.get(cardName);
      if (!existing || existing.synergy < synergy) {
  ...
  const priced = pool
    ...
    .filter((c): c is typeof c & { price: number } => c.price !== undefined && c.price > 0);
  if (priced.length === 0) {
...
export type { PriceConstrainedBuildResult };
