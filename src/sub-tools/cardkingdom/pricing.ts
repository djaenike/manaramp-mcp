/**
 * sub-tools/cardkingdom/pricing.ts
...
interface CardKingdomProduct {
  name?: string;
  edition?: string;
  is_foil?: boolean | string;
  qty_retail?: number | string;
  price_retail?: string;
  scryfall_id?: string;
}

interface CardKingdomPriceResult {
  name: string;
  edition?: string;
  is_foil: boolean;
  price_usd: number;
...
async function getCardKingdomPrice(name: string, include_foil?: boolean): Promise<CardKingdomPriceResult> {
  ...
  const products: CardKingdomProduct[] = body?.data ?? [];
  ...
  matches.sort((a, b) => parseFloat(a.price_retail ?? "0") - parseFloat(b.price_retail ?? "0"));
  ...
  return {
    ...
    price_usd: parseFloat(cheapest.price_retail ?? "0"),
...
interface DeckPriceTotalResult {
  total_usd: number;
  cards_priced: number;
  cards_not_found: string[] | undefined;
  note: string;
  breakdown: Array<{ name: string; price_usd: number }>;
}

/** Shared by get_deck_price_total and deliver_finished_deck (via delivery/deliver_finished_deck.js). */
async function computeDeckPriceTotal(cardNames: string[], includeFoil: boolean): Promise<DeckPriceTotalResult> {
  const res = await fetch(CARDKINGDOM_PRICELIST_URL, { headers: HEADERS });
  if (!res.ok) {
    throw new Error(`Card Kingdom pricelist request failed: ${res.status} ${res.statusText}`);
  }
  ...
  const priced: Array<{ name: string; price_usd: number }> = [];
...
async function fetchPriceByNameMap(): Promise<Map<string, number>> {
  const res = await fetch(CARDKINGDOM_PRICELIST_URL, { headers: HEADERS });
  if (!res.ok) {
    throw new Error(`Card Kingdom pricelist request failed: ${res.status} ${res.statusText}`);
  }
  ...
  for (const p of products) {
    ...
    if (!existing || price < existing) priceByName.set(name, price);
  }
  return priceByName;
}

export { CARDKINGDOM_PRICELIST_URL, getCardKingdomPrice, computeDeckPriceTotal, fetchPriceByNameMap };
export type { CardKingdomPriceResult, DeckPriceTotalResult };
