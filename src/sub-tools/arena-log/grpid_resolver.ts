...
/** A resolved card (or null for a confirmed miss), as returned by whatever search_cards-shaped
...
/** Matches search_cards' own signature: takes a Scryfall search query string, returns matches.
 * Deliberately `any[]` rather than a named card shape -- this module works with whatever
 * search_cards-shaped function is injected (real searchCards, or a fake in tests), and doesn't
 * need to know the exact result shape beyond "has a grpId-resolvable entry at [0]". */
type SearchCardsFn = (query: string) => Promise<any[]>;
...
interface ResolveGrpIdsResult {
  ...
  /**
   * grpId -> the actual failure message, for any grpId whose lookup threw (a genuine 404, a
   * network error, etc) -- present ONLY for entries resolved during this call, not for cache
   * hits (a cached failure's original reason isn't preserved).
   */
...
export type { ResolvedCard, SearchCardsFn, ResolveGrpIdsOptions, ResolveGrpIdsResult };
