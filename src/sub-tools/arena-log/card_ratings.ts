/**
 * sub-tools/arena-log/card_ratings.ts
...
type CardRatingRow = Record<string, string | number | null>;
...
function loadCardRatings(filePath: string): Map<string, CardRatingRow> {
  const csvText = readFileSync(filePath, "utf8");
  const rows = parseCardRatingsCsv(csvText);
  const byName = new Map<string, CardRatingRow>();
 ...
 */
function findLatestCsvInDir(dirPath: string): string | null {
  let entries: string[];
  try {
  ...
  const csvFiles = entries
    ...
    .filter((x): x is { full: string; mtimeMs: number } => x !== null);
