...
const readDeckTool = {
  name: "read_deck",
  description: "Read the calling account's decks. Pass deck_id or slug to load ONE deck's full detail -- real card names/mana costs/images, a ready-to-paste decklist_text (feed this straight into optimize_deck to start editing), mana curve, price, bracket, and consistency_issues as of its last publish_deck call. Omit both to list every deck on the account instead (name/slug/format/price/bracket/consistency_issues summary only, no per-card detail -- call again with a specific deck_id once you know which one). Read-only -- never persists anything; use optimize_deck then publish_deck (with this deck's deck_id) to actually change it.",
  inputSchema,
